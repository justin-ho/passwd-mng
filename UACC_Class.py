...
class UACC:
    ...
    # This function checks to see if the identifier meets our requirements.
    # Identifier inputs must not be: blank and only cointains letters
    ...
    def identifier_is_valid(self):
        if (any(str.isdigit(c) for c in self.identifier) == True or not self.identifier or re.search(r'[\s]',
                                                                                                     self.identifier)):
            return False
        else:
            return True

            # This function checks to see if the username meets our requirements.
            # Username must not be: blank and no longer than 16 characters long
